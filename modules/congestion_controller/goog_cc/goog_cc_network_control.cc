...
NetworkControlUpdate GoogCcNetworkController::OnNetworkRouteChange(
  ...
  if (AllowExternalBwe())
    ...
    delay_based_bwe_gcc_.reset(new DelayBasedBwe(
        key_value_config_, event_log_, network_state_predictor_.get()));
  }
#else
...
std::vector<ProbeClusterConfig> GoogCcNetworkController::ResetConstraints(
...
#ifdef INTEL_GPRA
    if (delay_based_bwe_gcc_)
      delay_based_bwe_gcc_->SetStartBitrate(*starting_rate_);
#endif
  }
  if (delay_based_bwe_)
    delay_based_bwe_->SetMinBitrate(min_data_rate_);
  ...
  if (AllowExternalBwe() && delay_based_bwe_) {
